...
await util.testExample("example/controller", async () => {
  if ((await $`curl -s http://localhost:8080`) != "hello world") throw "err";
  if ((await $`curl -s http://localhost:8080/home`) != "hello world")
    throw "err";
  if ((await $`curl -s http://localhost:8080/home/index`) != "hello world")
    throw "err";
  if ((await $`curl -s http://localhost:8080/blog`) != "blog list") throw "err";
  if ((await $`curl -s http://localhost:8080/blog/index`) != "blog list")
    throw "err";
  if ((await $`curl -s http://localhost:8080/about`) != "about") throw "err";
  if ((await $`curl -s http://localhost:8080/about/index`) != "about")
    throw "err";
  if ((await $`curl -s http://localhost:8080/api/v1`) != "here is api v1")
    throw "err";
  if ((await $`curl -s http://localhost:8080/api/v1/home`) != "here is api v1")
    throw "err";
  if (
    ...
    throw "err";
  if ((await $`curl -s http://localhost:8080/api/v1/blog`) != "blog api")
    throw "err";
  if ((await $`curl -s http://localhost:8080/api/v1/blog/index`) != "blog api")
    throw "err";
});
...
await util.testExample("example/cors", async () => {
  ...
  if (
    header.stdout.includes("Access-Control-Allow-Origin") &&
...
await util.testExample("example/group", async () => {
  if (
    (await $`curl -s http://localhost:8080/api/v1/hi`) !=
    "request path /api/v1/hi"
  )
    throw "err";
  if (
    (await $`curl -s http://localhost:8080/api/v1/hello`) !=
    "request path /api/v1/hello"
  )
    throw "err";
  if (
    (await $`curl -s http://localhost:8080/api/v2/hi`) !=
    "request path /api/v2/hi"
  )
    throw "err";
  if (
    (await $`curl -s http://localhost:8080/api/v2/hello`) !=
    "request path /api/v2/hello"
  )
    throw "err";
});
...
await util.testExample("example/view", async () => {
  ...
  if (
    !homeHtml.stdout.includes(">Header<") ||
    !homeHtml.stdout.includes(">Footer<")
  )
    throw "err";
});
