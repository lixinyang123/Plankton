var GLOBAL_FNS =
  'function _esc_(s){return (s+"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;");}\n' +
...
export default function codegen(tree, opts) {
  ...
  var stringify = function (tree) {
    ...
    tree.forEach(function (node) {
      if (node.type === "STATIC")
        return push(JSON.stringify(compress(node.value)));
      if (node.type === "EXPRESSION") return push("(" + node.value + ")");
      if (node.type === "ESCAPE_EXPRESSION")
        return push("_esc_(" + node.value + ")");
      ...
      global.push(
        "function " +
          id +
          "(locals){" +
          wrap(true, stringify(node.body) + "return _r;") +
          "}\n",
      );
      logic(
        "_b[" +
