...
export default function parse(src) {
  return Array.prototype.concat
    .apply(
      [],
      src.split(TOKEN_END).map(function (slice) {
    ...
    .map(function (data, i) {
      ...
      line =
        !line.length || (line[2] && !line[2])
          ? {}
          : {
              name: line[0],
              url:
                line[1] &&
                line[1].substr(1, line[1].length - 2).replace(/\\(.)/g, "$1"),
      ...
      if (ctx === "{}" && line.url)
        return { type: BLOCK_ANONYMOUS, url: line.url, locals: line.locals };
      if (ctx === "{" && line.name)
        return {
          type: auto,
          live: live,
    ...
    .reduce(function reduce(result, node) {
      ...
      last.body = last.capture
        ? last.body.concat(node)
        : last.body.reduce(reduce, []);
      return result;
    }, []);
}
