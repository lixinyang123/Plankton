...
export class Plankton {
  constructor() {
    ...
    this.server = http.createServer(async (req, res) => {
      ...
      await this.pipeline
        .invoke(Request.init(req, this.host), Response.init(res), this.pipeline)
        .catch((error) => {
  ...
  async #endpoint(req, res) {
    let action = this.route.find(req.pathname);
  ...
  map(routePath, callback, method = "") {
    switch (Object.prototype.toString.call(callback)) {
      case "[object AsyncFunction]":
        this.actions.push(this.route.register(routePath, callback, method));
        break;
  ...
  mapController(dir = "controllers", basePath = "/") {
    ...
    fs.readdirSync(controllerDir).forEach(async (controllerName) => {
      ...
      if (module["index"]) {
        this.map(
          path.join(basePath, controllerName.replace(".js", ""), "/?"),
          module["index"],
        );
      }
